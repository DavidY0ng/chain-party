<script lang="ts">
	...
	$: if (($rerender || !$rerender) && gameRoundData) {
		if (api) {
			...
			timeoutId = setTimeout(() => {
				api.scrollTo(gameRoundData?.data.length - 2);
			}, 10000);

			// on gameRoundData changed then scroll to latest index
			api.on('slidesChanged', () => {
				if (current - gameRoundData?.data.length === -4) {
					api.scrollTo(gameRoundData?.data.length - 2);
				}
			});
		}
...
<Carousel.Root
	...
	opts={{
		align: 'start',
		startIndex: gameRoundData?.data.length - 2
	}}
